...
//#include "RooHybridTailsGaus.h"

 
using namespace RooFit;

 
void eregtrainingFAlpha_Log(bool dobarrel=true,Double_t minCutSig = 3, Double_t nmax=500, bool doele=false) {

	//values param
	Double_t n1max=nmax, n2max=nmax;
	...
	//output directory 
	if (dobarrel)
		dirname = "Barrel_Log_sig"+minCutSigS+"_FAlpha_"+"_evts"+cutEvtsS+"/"; 
	else
		dirname = "Endcap_Log_sig"+minCutSigS+"_FAlpha"+"_evts"+cutEvtsS+"/";
  	...
  	else
	{
 	...
 	tgts.add(*sigwidtht);
  	...
  	RooRealConstraint siga1lim("siga1lim","",*siga1t,0.05,10.); 
       RooRealConstraint sign1lim("sign1lim","",*sign1t,1.01,n1max); 
  	RooRealConstraint siga2lim("siga2lim","",*siga2t,0.05,10.); 
  	RooRealConstraint sign2lim("sign2lim","",*sign2t,1.01,n2max); 
  	...
  	RooDoubleCBFast sigpdf("sigpdf","",*tgtvar,sigmeanlim,sigwidthlim,siga1lim,sign1lim,siga2lim,sign2lim);
  	//RooHybridTailsGaus sigpdf("sigpdf","",*tgtvar,sigmeanlim,sigwidthlim,siga1lim,siga2lim,sign2lim);
  	...
  	if (1) 
	{
  		...
  		//bdtpdfdiff.SetPrescaleInit(100);
  	  	...
  	  	bdtpdfdiff.SetMaxNodes(750); //750
