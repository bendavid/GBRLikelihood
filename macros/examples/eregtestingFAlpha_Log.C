...
void eregtestingFAlpha_Log(bool dobarrel=true, bool doele=false) {
  
  	//output dir
  	TString dirname = "Barrel_Log_sig3_FAlpha__evts15/";
  	gSystem->mkdir(dirname,true);
  	gSystem->cd(dirname);    

	TCut cutUsed = "(event%15==1)";
	Double_t nmax=500.,nbinsn=500;
	Double_t mintgtvar = -0.5, nBins1 = 500, nBins2=250,nBins3 =800;
	...
	//}
  	...
  	if (dobarrel) 
	{
    		selcut = "etrue*pt/energy >=200. && etrue*pt/energy<=3000 && abs(scEta)<1.5 && kSaturated[12]!=1 ";
  	}
  	else
	{
    		selcut = "etrue*pt/energy >=200. && etrue*pt/energy<=3000 && abs(scEta)>1.5 && kSaturated[12]!=1 " ;   
	...
	RooAbsReal *siga1lim = ws->function("siga1lim");
	RooAbsReal *siga2lim = ws->function("siga2lim");
	...
	RooRealVar *a1var = (RooRealVar*)hdataclone->addColumn(*siga1lim);
  	RooRealVar *a2var = (RooRealVar*)hdataclone->addColumn(*siga2lim);
	...
	TCanvas *craw = new TCanvas("craw","craw",800,600); //------------------------------------------------------------CANVAS
  	...
  	RooPlot *plot = tgtvar->frame(mintgtvar,.6, nBins2);
  	plot->SetTitle("");
	...
	gPad->SetGridy();
  	...
  	plot->Draw();
	TLegend *legend = new TLegend(0.11,0.76,0.3,0.89);
...
/*
   ...
   /*    TCanvas *craw3 = new TCanvas("craw3","craw3"); //------------------------------------------------------------CANVAS
	...
	TCanvas *ca1 = new TCanvas("ca1","ca1"); //------------------------------------------------------------------CANVAS
  	//RooPlot *plota1 = nvar->frame(Title("hdata vs nvar"),0.,111.,200);
  	//RooPlot *plota1 = nvar->frame(0.,111.,200);
	RooPlot *plota1 = a1var->frame(0.05,10,1000);
  	plota1->SetTitle("Alpha1 distribution");
  	plota1->GetXaxis()->SetTitle("alpha1");
  	hdataclone->plotOn(plota1);
  	plota1->Draw();
  	ca1->SaveAs("a1"+add1);
       
	TCanvas *ca2 = new TCanvas("ca2","ca2"); //------------------------------------------------------------------CANVAS
  	//RooPlot *plota2 = nvar->frame(Title("hdata vs nvar"),0.,111.,200);
  	//RooPlot *plota2 = nvar->frame(0.,111.,200);
	RooPlot *plota2 = a2var->frame(0.05,10,1000);
  	plota2->SetTitle("Alpha2 distribution");
  	plota2->GetXaxis()->SetTitle("alpha2");
  	hdataclone->plotOn(plota2);
  	plota2->Draw();
  	ca2->SaveAs("a2"+add1);
  	...
  	//RooPlot *plotn1 = nvar->frame(0.,111.,200);
       //nmax
	RooPlot *plotn1 = n1var->frame(0,nmax,nbinsn);
  	...
  	RooPlot *plotn2 = n2var->frame(0,nmax,nbinsn);
...
//create canvas for mean_of_gaussian
  ...
  /*     TObjArray* Arr = new TObjArray();
	...
	canvas_ecor_fit->cd(1);       
       ...
       canvas_ecor_fit->cd(2);       
       plot_Slices(hdata,dobarrel,Arr,r9,ecorvar, rawvar,"hdata_hist_00000014_1","hdata_hist_00000016_chi2","R9","energy/generated_energy",plotEMin, plotEMax,0.7,1);
	...
	canvas_ecor_fit->cd(3);       
       plot_Slices(hdata,dobarrel,Arr,pt,ecorvar, rawvar,"hdata_hist_00000016_1","hdata_hist_0000001c_chi2","Pt","energy/generated_energy",plotEMin, plotEMax,0.,5000,0.,5000);
	...
	canvas_ecor_fit->cd(4);       
       plot_Slices(hdata,dobarrel,Arr,rho,ecorvar, rawvar,"hdata_hist_00000018_1","hdata_hist_00000020_chi2","rho","energy/generated_energy",plotEMin, plotEMax,0.,30,0.,30);
