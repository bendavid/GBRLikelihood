...
void resolution_effsigmaCB_RawCor(bool dobarrel=true, bool weighted=false) {
  
  	//output dir
  	TString dirname = "Barrel_Log_sig3_EvenOddEvts_alpha2-3/";
      ...
      //regressed output functions
       ...
       if(!dobarrel) {
	       ...
	       //formula for Pt corrected_energy/Cosh(eta) = ecorvar*tgtvar*rawvar/cosh(eta) 
  	       RooFormulaVar ptFormula("pt","","exp(@0)*@1*(1+@2)*1./(cosh(@3))",RooArgList(*sigmeanlim,*scRawEnergy,*preshowerEnergy,*scetavar));
     	       pt = (RooRealVar*)hdata->addColumn(ptFormula);
  	       pt->setRange(250.,5000); //
       }
       else {
              RooFormulaVar ptFormula("pt","","exp(@0)*@1*1./(cosh(@2))",RooArgList(*sigmeanlim,*scRawEnergy,*scetavar));
     	       pt = (RooRealVar*)hdata->addColumn(ptFormula);
  	       pt->setRange(250.,5000); //
       }
       ...
       plot_resolution("pt",0.,5000.,nbins, xbins, step, init, "pT (GeV)", add_to_pngFileName, hdata, rawvar,ecorvar, c_reso);
       c_reso->SaveAs("resolution_vs_pt_CMS.png");
       c_reso->SaveAs("resolution_vs_pt_CMS.root");
}
void plot_resolution(TString arg_variable, Double_t xmin, Double_t xmax, Int_t nbins, Double_t* xbins, Double_t step, Double_t init, TString nameXaxis, TString add_to_pngFileName, RooDataSet* hdata, RooRealVar* rawvar, RooRealVar* ecorvar, TCanvas* c_reso)
{
       ...
       for(Int_t i=0; i<nbins; i++) {
	       ...
	       TH1 *hraw = hdata_reduced->createHistogram("hraw",*rawvar,Binning(20e3,0.,4.));
         	...
         	pdfCB_cor.fitTo(*hdata2,Range(0.9,1.1));
              ...
              h_gaussian_cor->SetBinError(i+1,100.*sig_cor.getError());
         	...
         	pdfCB_raw.fitTo(*hdata3,Range(0.6,1.1));
         	
              h_gaussian_raw->AddBinContent(i+1,100.*sig_raw.getVal());
              h_gaussian_raw->SetBinError(i+1,100.*sig_raw.getError());
