...
using namespace RooFit;
 ...
 RooCBFast::RooCBFast(const char *name, const char *title, 
                    RooAbsReal& _x,
                    RooAbsReal& _mean,
                    RooAbsReal& _width,
                    RooAbsReal& _alpha1,
 ...
 RooCBFast::RooCBFast(const RooCBFast& other, const char* name) :  
 ...
 double RooCBFast::evaluate() const 
 { 
...
//    if (!std::isnormal(val)) {
//      printf("bad val: x = %5f, t = %5f, mean = %5f, sigma = %5f, alpha1 = %5f, n1 = %5f\n",double(x), t, double(mean),double(width),double(alpha1), double(n1));
//      printf("val = %5f\n",val);
//    }
 ...
 Int_t RooCBFast::getAnalyticalIntegral(RooArgSet& allVars, RooArgSet& analVars, const char* range) const 
 {
   if (matchArgs(allVars,analVars,x)) return 1;
   return 0;
 }

 Double_t RooCBFast::analyticalIntegral(Int_t code, const char* rangeName) const 
 {
   assert(code==1) ;
...
//    if (!std::isnormal(sum)) {
//      printf("bad int: mean = %5f, sigma = %5f, alpha1 = %5f, n1 = %5f\n",double(mean),double(width),double(alpha1), double(n1));
